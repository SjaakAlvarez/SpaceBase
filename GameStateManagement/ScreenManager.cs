...
namespace GameStateManagement
{
    ...
    public class ScreenManager : DrawableGameComponent
    {
        ...
        public RenderTarget2D levelTarget;
        ...
        protected override void LoadContent()
        {
            ...
            levelTarget = new RenderTarget2D(GraphicsDevice, (int)Constants.WIDTH, (int)Constants.HEIGHT, false, GraphicsDevice.PresentationParameters.BackBufferFormat, DepthFormat.None);
            shader = content.Load<Effect>("Effects/ScanLine");

            if (GameSettings.getBool("Graphics.Vertical"))
            {
                shader.Parameters["width"].SetValue((float)GameSettings.getInt("Graphics.Height"));
                shader.Parameters["height"].SetValue((float)GameSettings.getInt("Graphics.Width"));
            }
            else
            {
                shader.Parameters["height"].SetValue((float)GameSettings.getInt("Graphics.Height"));
                shader.Parameters["width"].SetValue((float)GameSettings.getInt("Graphics.Width"));
            }

            shader.Parameters["targetwidth"].SetValue(Constants.WIDTH * ((Game1)Game).camera.Zoom);
            shader.Parameters["OFF_INTENSITY"].SetValue(GameSettings.getFloat("HLSL.ScanLineOffIntensity"));
            shader.Parameters["CurvatureAmount"].SetValue(GameSettings.getFloat("HLSL.Curvature"));
            shader.Parameters["redShift"].SetValue(GameSettings.getFloat("HLSL.RedShift"));
            shader.Parameters["greenShift"].SetValue(GameSettings.getFloat("HLSL.GreenShift"));
        }
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            Matrix transform = ((Game1)Game).camera.get_transformation(GraphicsDevice);

            if (GameSettings.getBool("HLSL.Enabled"))
            {
                GraphicsDevice.SetRenderTarget(null);
                spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.Opaque, SamplerState.AnisotropicClamp, null, null, shader, transform);
                GraphicsDevice.Clear(Color.Black);
                spriteBatch.Draw(levelTarget, new Rectangle(0, 0, (int)Constants.WIDTH, (int)Constants.HEIGHT), new Rectangle(0, 0, (int)Constants.WIDTH, (int)Constants.HEIGHT), Color.White);
                spriteBatch.End();
            }
            else
            {               
